...
class RolloutBuffer:
    ...
    def append(self, state, action, reward, done, log_pi, next_state):
        ...
        # Total number of data tuple in the buffer.
    ...
    def sample(self, batch_size):
        """
        Sample for training Disc in GAIL.
        """
        assert self._p % self.buffer_size == 0
        idxes = np.random.randint(low=0, high=self._n, size=batch_size)
        return (
...
class RolloutBufferTypeA:
    ...
    def __init__(self, buffer_size, state_shape, action_shape, device, mix=1):
        ...
        self.next_states = torch.empty(
            (self.total_size, *state_shape), dtype=torch.float, device=device)
        self.loc = torch.empty(
            (self.total_size, *action_shape), dtype=torch.float, device=device)
        self.scale = torch.empty(
            (self.total_size, *action_shape), dtype=torch.float, device=device
        )

    def append(self, state, action, reward, done, log_pi, next_state, loc, scale):
        self.states[self._p].copy_(torch.from_numpy(state))
        self.actions[self._p].copy_(torch.from_numpy(action))
        self.rewards[self._p] = float(reward)
        self.dones[self._p] = float(done)
        self.log_pis[self._p].copy_(torch.from_numpy(log_pi))
        self.next_states[self._p].copy_(torch.from_numpy(next_state))
        self.loc[self._p].copy_(loc.squeeze(0))
        self.scale[self._p].copy_(scale.squeeze(0))
...
class RolloutBufferTypeB:
    ...
    def __init__(self, buffer_size, state_shape, action_shape, device, mix=1):
        ...
        self.log_pis = torch.empty((self.total_size, *action_shape), dtype=torch.float, device=device)
        self.next_states = torch.empty((self.total_size, *state_shape), dtype=torch.float, device=device)
        self.loc = torch.empty((self.total_size, *action_shape), dtype=torch.float, device=device)
        self.scale = torch.empty((self.total_size, *action_shape), dtype=torch.float, device=device)
        self.beliefs = torch.empty((self.total_size, 1), dtype=torch.float, device=device)

    def append(self, state, action, reward, done, log_pi, next_state, loc, scale, belief):
        self.states[self._p].copy_(torch.from_numpy(state))
        self.actions[self._p].copy_(torch.from_numpy(action))
        self.rewards[self._p] = float(reward)
        self.dones[self._p] = float(done)
        ...
        self.loc[self._p].copy_(loc.squeeze(0))
        self.scale[self._p].copy_(scale.squeeze(0))
        self.beliefs[self._p] = float(belief)
